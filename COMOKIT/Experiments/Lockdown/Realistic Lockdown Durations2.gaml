...
experiment "Early containment" parent: "Abstract Experiment" autorun: true {
	...
	action _init_ {
		...
		float tolerance <- 0.1;
		int nb_tests_ <- 100;
		loop nbI over: [2,4,6,8] {

		// The possible durations to be compared are 0, 15, 30, 45 and 60 days.
			loop nb_days over: [15] {
				create simulation with: [num_infected_init::nbI, color::(colors at int(color_browser)), dataset_path::shape_path, seed::simulation_seed] {
					name <- string(nb_days) + " days containment after " + nb_cases + " cases, "+" infected sources: "+nbI;
					ask Authority {
